#!/usr/bin/env python3
"""
Fixed training + backtest for State-Dependent NHP v2.

Fixes from previous attempt:
1. Actually fits HMM RegimeDetector and uses real regime probabilities
2. Clips vol features to prevent tanh saturation (cap at 5x median)
3. Uses state_vector_at() from nhp_regime for proper state construction
4. Trains on all available timeframes (1h, 4h, 1d) for NQ + ES
5. Proper temporal train/val split with no leakage
"""
...
# ── State Vector Construction (with proper HMM) ───────────────────────────────

def build_state_vectors_for_bars(
    prices: np.ndarray,
    ...
    """
    Build (N, 6) state vectors for each bar using fitted HMM.
    Clips vol features to prevent tanh saturation.
    """
    vf = compute_vol_features(prices, bar_times)
    regimes, probs = regime_detector.predict(vf)
    
    n = len(prices)
    states = np.zeros((n, 6), dtype=np.float32)
    
    # Normalize by median (only positive values)
    pos_rv = vf.realized_vol[vf.realized_vol > 0]
    median_rv = float(np.median(pos_rv)) if len(pos_rv) > 0 else 1e-8
    
    for i in range(n):
        sv = state_vector_at(vf, regimes, probs, i)
        # Normalize and clip vol features
        sv[0] = np.clip(sv[0] / (median_rv + 1e-8), 0, clip_max)  # realized vol
        sv[1] = np.clip(sv[1] / (median_rv + 1e-8), 0, clip_max)  # vol of vol
        sv[2] = np.clip(abs(sv[2]), 0, clip_max)  # |skew|
        # sv[3:6] are regime probs from HMM (already normalized)
        states[i] = sv
    
    return states, regimes, vf
...
def build_sequences(
    ...
    """Build event sequences with state vectors from OHLC data."""
    prices = df['Close'].values.astype(np.float64)
    bar_times = np.arange(len(prices), dtype=np.float64)
    
    # Compute state vectors with fitted HMM
    states, regimes, vf = build_state_vectors_for_bars(
        prices, bar_times, regime_detector, clip_max
    )
    ...
    for start in range(0, n_bars - window_size + 1, stride):
        ...
        # States are offset by +1 because dts[i] = timestamp[i+1] - timestamp[i]
        seq_states = states[start + 1:end + 1].astype(np.float32)
        if len(seq_states) < window_size:
            pad = np.zeros((window_size - len(seq_states), 6), dtype=np.float32)
            seq_states = np.vstack([seq_states, pad])
        sequences.append({
            'dts': seq_dts,
            'types': seq_types,
            'states': seq_states,
...
def train_epoch(model, loader, optimizer, device, n_mc=3):
    ...
    for batch in loader:
        ...
        optimizer.zero_grad()
        ll = model.log_likelihood(types, dts, states, lengths, n_mc=n_mc)
        loss = -ll
        loss.backward()
...
def run_inference(
    ...
    """Run NHP v2 inference on OHLC data with proper state vectors."""
    prices = df['Close'].values.astype(np.float64)
    bar_times = np.arange(len(prices), dtype=np.float64)
    
    # Build state vectors with fitted HMM
    states, regimes, vf = build_state_vectors_for_bars(
        prices, bar_times, regime_detector, clip_max
    )
    ...
    # State vectors for each dt (offset by 1)
    state_vecs = states[1:n_bars + 1]
    if len(state_vecs) < n_bars:
        pad = np.zeros((n_bars - len(state_vecs), 6), dtype=np.float32)
        state_vecs = np.vstack([state_vecs, pad])
    ...
    # Windowed inference — match training window size (20 bars)
    # Run model on sliding windows, average overlapping predictions
    window_size = 20
    stride = 10
    ...
    with torch.no_grad():
        ...
        if n_bars > window_size:
            ...
            w_states = torch.tensor(state_vecs[remaining_start:n_bars], dtype=torch.float32).unsqueeze(0)
    ...
    if m_std > 1e-8:
        ...
        model_norm = np.clip(model_zscore, -3, 3) / 6.0 + 0.5  # → [0, 1] centered at 0.5
    else:
...
def main():
    print("=" * 60)
    print("  TRAINING STATE-DEPENDENT NHP v2 (FIXED)")
    print("=" * 60)
    ...
    for symbol in ['NQ', 'ES']:
        for tf, rel_path in TRAIN_FILES[symbol].items():
            ...
            offset += len(prices) + 100  # gap between datasets
            print(f"    {symbol} {tf}: {len(df)} bars")
    
    # Concatenate for HMM fitting
    concat_prices = np.concatenate(all_prices)
    concat_times = np.concatenate(all_bar_times)
    
    # ── Step 2: Fit HMM on training portion (first 80%) ──
    ...
    # ── Step 3: Build sequences with proper state vectors ──
    print("\n  Building training sequences...")
    all_sequences = []
    
    for key, df in all_dfs.items():
        seqs = build_sequences(df, detector, window_size=20, stride=10, clip_max=5.0)
        all_sequences.extend(seqs)
        print(f"    {key}: {len(seqs)} sequences")
    
    print(f"\n  Total sequences: {len(all_sequences)}")
    
    # Verify state vector quality
    sample_states = all_sequences[0]['states']
    print(f"  Sample state vector stats:")
    for d in range(6):
        vals = np.array([s['states'][:, d] for s in all_sequences[:100]]).flatten()
        print(f"    dim {d}: min={vals.min():.3f} max={vals.max():.3f} mean={vals.mean():.3f} std={vals.std():.3f}")
    
    # ── Step 4: Create dataloaders ──
    # Temporal split: first 80% train, next 10% val, last 10% test
    n = len(all_sequences)
    n_train = int(n * 0.8)
    n_val = int(n * 0.1)
    ...
    train_ds = StateAwareDataset(train_seqs, max_len=20)
    val_ds = StateAwareDataset(val_seqs, max_len=20)
    ...
    # ── Step 5: Train ──
    print(f"\n  Training StateDependentNHP v2...")
    cfg = NHPv2Config(embed_dim=16, hidden_dim=32, state_dim=6, state_hidden=8)
    model = StateDependentNHP(cfg).to(device)
    ...
    # ── Step 6: Backtest ──
    print("\n" + "=" * 60)
    print("  BACKTEST WITH TRAINED v2 MODEL")
    print("=" * 60)
    ...
    for symbol in ['NQ', 'ES']:
        ...
        for tf, rel_path in BACKTEST_FILES[symbol].items():
            ...
            try:
                ...
                result = run_inference(df, detector, CHECKPOINT, blend_weight=0.4, clip_max=5.0)
                if result is None:
...
def write_results(results):
    out = "/home/ubuntu/nhp_v2_backtest_results.md"
    with open(out, 'w') as f:
        f.write("# NHP v2 Backtest Results — State-Dependent Model + Regime Gate (FIXED)\n\n")
        f.write("**Fixes applied:**\n")
        f.write("- HMM regime detector properly fitted on training data\n")
        f.write("- Real regime probabilities used in state vectors (not hardcoded)\n")
        f.write("- Vol features clipped to prevent tanh saturation\n")
        f.write("- Blend weight 0.4 (model) / 0.6 (price activity)\n\n")
        ...
        f.write("| Symbol | TF | Bars | Raw→Gated | Overall | Enter | Exit | λ_std |\n")
        f.write("|--------|-----|------|-----------|---------|-------|------|-------|\n")
        for r in results:
            f.write(f"| {r['symbol']} | {r['tf']} | {r['bars']} | "
                    f"{r['raw_signals']}→{r['gated_signals']} | "
