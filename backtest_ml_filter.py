"""
Test whether a simple ML filter (VWAP-distance normalized by a GARCH
volatility forecast) adds real, out-of-sample edge on top of VWAP/TDA/GMM
candidate levels - as opposed to another level-DISCOVERY method (which is
the thing already disproven four separate times in this project).

Design:
  1. Walk forward. At each window, fit GARCH once (shared across all three
     level types at that window) and compute session VWAP once.
  2. Generate candidates from VWAP itself, TDA (persistent homology), and
     GMM. For each touched candidate, record bounce/break outcome plus
     filter features: distance from VWAP normalized by the GARCH forecast,
     the raw vol forecast, ATR-distance, and level type.
  3. Chronological split per file (first 70% train, last 30% holdout) -
     PIT-safe: the logistic regression is fit only on the train period.
  4. On holdout: compare each level type's UNCONDITIONAL accuracy against
     its ML-FILTERED accuracy (top half by predicted P(bounce)), each
     tested for significance against the random baseline with the same
     z-test/Bonferroni discipline as every other result in this project.
"""
import argparse
import contextlib
import io
...
def collect_events(path, lookback, horizon, step,
    ...
    for wi, t in enumerate(starts):
        ...
        candidates = (
            [(vwap, 'VWAP')] +
            [(lvl['price'], 'TDA') for lvl in (tda_levels or [])] +
            [(lvl['price'], 'GMM') for lvl in (gmm_levels or [])]
        )
...
def main():
    ...
    ap.add_argument('--out', default='backtest_ml_filter_events.csv')
    ...
    n_comparisons = 3  # VWAP, TDA, GMM filtered results, tested this round
